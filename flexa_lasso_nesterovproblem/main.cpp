...
//Functions
double compute_lipschitz_constant(double tolerance, int m, int p, double *A) {
	double L = 0.0, local_norm = 0.0, local_error = 0.0, reference_error = 0.0, norm_x = 0.0, norm_y = 0.0, global_error = 0.0;
	double *Ax_local = new double[m]();
	double *Ax_global = new double[m]();
	double *column_sum_vector = new double [p]();  
	for (int i = 0; i < p; i++) {
		for (int j = 0; j < m; j++) 
		    column_sum_vector[i] += std::abs(A[i*m+j]); 		
		local_error += column_sum_vector[i]*column_sum_vector[i];
	}
	MPI_Allreduce(&local_error, &global_error,  1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);	
	global_error = std::sqrt(global_error);
	column_sum_vector[0:p:1] = (1/global_error)*column_sum_vector[0:p:1]; 
	while (std::abs(global_error-reference_error) > tolerance*global_error) {
		reference_error = global_error;
		cblas_dgemv (CblasColMajor, CblasNoTrans, m, p, 1, A, m, column_sum_vector, 1, 0, Ax_local, 1);
		MPI_Allreduce(Ax_local, Ax_global, m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		cblas_dgemv (CblasColMajor, CblasTrans, m, p, 1, A, m, Ax_global, 1, 0, column_sum_vector, 1);
		local_norm = 0; 
		norm_y = 0;
		for (int i = 0; i < p; i++) 
		    local_norm += column_sum_vector[i]*column_sum_vector[i]; 	
		for (int j = 0; j < m; j++)
		    norm_y = Ax_global[j]*Ax_global[j];
		MPI_Allreduce(&local_norm, &norm_x, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		norm_x = std::sqrt(norm_x);
		norm_y = std::sqrt(norm_y);
		global_error = norm_x/norm_y;
		column_sum_vector[0:p:1] = (1/norm_x)*column_sum_vector[0:p:1]; 
	}
	L = global_error*global_error;
	delete [] Ax_local;	delete [] Ax_global; delete [] column_sum_vector; 	
	return L;
}
...
void compute_new_point(int i, double prox, double lambda, double step, double *x, double *gradient, double *hessian) {	
    double x_opt = 0.0, parameter = 0.0;
...
int main (int argc, char **argv) {	
	...
	double L = 0.0, entry = 0.0, f_star = 0.0, f_value = 0.0, f_value_old = 0.0, m_value = 0.0, hds_local = 0.0, hds_global = 0.0, prox =0.0;
	...
	data = "data/xs.dat";
	file = fopen(data.c_str(), "r");
	double *x_star = new double[p]();
	position = 0;
	for (int i = 0; i < n; i++) {
		column = i;		
		fscanf(file, "%lf", &entry); 
		if(p*(rank+1) > column && column >= p*rank) { 
		    x_star[position] = entry; 
		    position++; 
		}
		fscanf(file, "\n");
	}
	...
	double *Ax = new double[m](); 
	double g_local = 0.0, g_global = 0.0, f =0.0; 
	cblas_dgemv (CblasColMajor, CblasNoTrans, m, p, 1, A, m, x_star, 1, 0, Ax, 1);
	MPI_Allreduce(Ax, F, m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); 
	for(int i = 0; i < m; i++){
	    F[i] -= b[i];
	    f += F[i]*F[i];
	...
	L = compute_lipschitz_constant(lip_tol, m, p, A);
	...
	while (iter_flag == 0 && merit_flag == 0 && time_flag == 0) {	
        ...
        f_value_old = f_value;
		for (int i = 0; i < p; i++)					
		    compute_new_point(i, prox, lambda, step, x, gradient, hessian);				
